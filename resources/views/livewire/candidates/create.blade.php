<div x-data="{ openModal: false }">
    {{-- <button @click="openModal = !openModal" class="text-sm rounded-lg px-4 py-2 bg-gray-500 text-white hover:bg-gray-400 focus:outline focus:outline-gray-500 active:bg-gray-600">Add Team</button>
     --}}
     <x-primary-button @click="openModal = !openModal">Add Candidate</x-primary-button>
    <div x-show="openModal" class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
          <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            wire:submit.prevent="createCandidate"
            x-show="openModal"
            x-trap="openModal"
            ...
            @click.outside="openModal = !openModal" class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
              <div class="bg-gray-700 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                  ...
                  <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 class="text-lg font-medium leading-6 text-gray-100" id="modal-title">Add Candidate</h3>
                    <div class="mt-2 text-gray-200">
                        <div class="mb-4">
                            <label class="block uppercase text-xs font-bold mb-2" for="firstname">First Name</label>
                            {{-- <input wire:model="name" id="name" class="w-full" type="text" name="name">
                             --}}
                             <x-text-input class="w-full" wire:model="firstname" id="firstname" type="text" name="firstname" :value="old('firstname')" autofocus autocomplete="firstname" ></x-text-input>
                            @error('firstname') <span class="text-red-400 text-xs font-bold">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            <label class="block uppercase text-xs font-bold mb-2" for="lastname">Last Name</label>
                            {{-- <input wire:model="name" id="name" class="w-full" type="text" name="name">
                             --}}
                             <x-text-input class="w-full" wire:model="lastname" id="lastname" type="text" name="lastname" :value="old('lastname')" autofocus autocomplete="lastname" ></x-text-input>
                        ...
                        <div class="mb-4">
                            <label class="block uppercase text-xs font-bold mb-2" for="gender">Gender</label>
                            {{-- <input wire:model="name" id="name" class="w-full" type="text" name="name">
                             --}}
                             <x-text-input class="w-full" wire:model="gender" id="gender" type="text" name="gender" :value="old('gender')" autofocus autocomplete="gender" ></x-text-input>
                            @error('gender') <span class="text-red-400 text-xs font-bold">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            <label class="block uppercase text-xs font-bold mb-2" for="team_id">Team</label>
                            {{-- <input wire:model="number" type="number" name="number" class="w-full appearance-none"> --}}
                            <select wire:model="team_id" name="team_id" id="team_id" class="w-full appearance-none border-gray-600 focus:border-gray-600 focus:ring-gray-600 rounded-md shadow-sm bg-gray-800 text-gray-400">
                                @foreach($teams as $team)
                                    <option value="{{ $team->id }}">{{ $team->monickerName() }}</option>
                                @endforeach
                            </select>
                            @error('team_id') <span class="text-red-400 text-xs font-bold">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                          <label class="block uppercase text-xs font-bold mb-2" for="photo">Upload Photo</label>
                          <x-text-input class="w-full file:bg-gray-600 file:text-gray-100 file:py-2 file:px-2 file:hover:bg-gray-500 file:overflow-hidden file:border-0" wire:model="photo" id="photo" type="file" name="photo" :value="old('photo')"></x-text-input>
                          @error('color') <span class="text-red-400 text-xs font-bold">{{ $message }}</span> @enderror
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="bg-gray-700 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                ...
                <x-primary-button class="sm:ml-3">Add Candidate</x-primary-button>
